...
export default function Dashboard() {
  ...
  const [drafts, setDrafts] = useState<
    Record<string, { status: string; revenue: string; source: string }>
  >({});
  ...
  async function load(
    ...
    try {
      ...
      const nextDrafts: Record<
        string,
        { status: string; revenue: string; source: string }
      > = {};
  ...
  async function saveLead(lead: Lead) {
    ...
    try {
      ...
      if (json?.capi?.purchase?.ok && !json.capi.purchase.skipped) {
        ...
        setNotice(
          `Tersimpan: Touch/Trigger ${json.lead.source} • Status ${json.lead.status}.`
        );
      }

      // Keep the returned DB value immediately so the UI does not appear
      // to revert while the list is being refreshed.
      setDrafts((current) => ({
        ...current,
        [lead.id]: {
                  ...
                  return (
                    <tr key={lead.id}>
                      ...
                      <td style={{ minWidth: 170 }}>
                        ...
                        >
                          <option value="Meta Ads">Meta Ads</option>
                          <option value="WhatsApp Organic">
                            WhatsApp Organic
                          </option>
                        </select>

                        {lead.source_id && draft.source !== "Meta Ads" && (
                          <div
                            className="sub"
                            style={{ color: "#b54708", maxWidth: 180 }}
                          >
                            Ada Ad ID, saat disimpan sistem akan menormalkan
                            kembali menjadi Meta Ads.
                          </div>
                        )}
                      </td>
