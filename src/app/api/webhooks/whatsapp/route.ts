  ...
  try {
    ...
    for (const entry of payload?.entry ?? []) {
      for (const change of entry?.changes ?? []) {
        ...
        for (const contact of value?.contacts ?? []) if (contact?.wa_id) contactByWaId.set(String(contact.wa_id), contact);
        ...
        for (const message of value?.messages ?? []) {
          ...
          const sourceId = referral?.source_id ? String(referral.source_id) : null;

          let metaAttribution: any = null;
          if (sourceId) {
            try { metaAttribution = await fetchMetaAdAttribution(sourceId); }
            catch (error) { console.error("Meta enrichment failed:", error); }
          }

          const { data: existing, error: findError } = await db.from("leads").select("*").eq("wa_id", waId).maybeSingle();
          ...
          const attributionPatch = sourceId ? {
            ...
            meta_enriched_at: metaAttribution ? new Date().toISOString() : null
          } : {};

          if (!existing) {
            const { data: inserted, error: insertError } = await db.from("leads").insert({
              wa_id: waId,
              phone: `+${waId}`,
              name,
              status: "Chat Builder",
            ...
            leadId = existing.id;
            const patch: Record<string, any> = { name: existing.name || name, last_message: messageText, last_seen_at: messageTime };
            if (sourceId && !existing.source_id) Object.assign(patch, attributionPatch);
            else if (existing.source_id && !existing.ad_name && metaAttribution) Object.assign(patch, {
              ad_name: metaAttribution.ad_name ?? null,
              adset_id: metaAttribution.adset_id ?? null,
              adset_name: metaAttribution.adset_name ?? null,
              campaign_id: metaAttribution.campaign_id ?? null,
            ...
            const { error: updateError } = await db.from("leads").update(patch).eq("id", leadId);
